...
class YITHWooCommerceAddOnsExtraPremiumOptions {
	...
	public function product_addons_set_cart_contents( $cart_contents, $cart_item_key, $cart_item, $apply_currency ) {
		if ( isset( $cart_item['yith_wapo_options'] ) && ! empty( $cart_item['yith_wapo_options'] ) ) {
			...
			SupportHelper::set_cart_item_objects_property( $cart_contents[ $cart_item_key ]['data'], 'yay_yith_wapo_set_product_price_with_options_default', SupportHelper::get_product_price( $product_id ) + (float) $price_options / YayCurrencyHelper::get_rate_fee( $this->apply_currency ) );
